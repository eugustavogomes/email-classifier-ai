...
export default function Result({ result }: { result: ApiResult }) {
  if (result.erro)
    return (
      <div className="card animate-slide-up flex items-start gap-3 text-red-600">
        ...
        <div>
          <h3 className="font-semibold mb-1">Erro no processamento</h3>
          <p>{result.erro}</p>
        </div>
      </div>
    );
  ...
  return (
    <div className="card animate-slide-up space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-gray-600 mb-2">CLASSIFICAÇÃO</h3>
        <div className="flex items-center gap-3">
          <span className={result.categoria === "Produtivo" ? "badge-success text-lg" : "badge-warning text-lg"}>
            {result.categoria === "Produtivo" ? "✓ Produtivo" : "⊘ Improdutivo"}
          </span>
          {result.confianca && (
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
        <h3 className="text-sm font-semibold text-blue-900 mb-2">💬 RESPOSTA SUGERIDA</h3>
      ...
      {result.stats && (
        <div className="grid grid-cols-3 gap-4 pt-4 border-t">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-600">Relevantes</p>
          </div>
          <div className="text-center">
